...
if (0 < $arResult['SECTIONS_COUNT'])
{
    ...
    $rsSections = CIBlockSection::GetList(array(), array('IBLOCK_ID'=>$arParams['IBLOCK_ID'],'ID' => array_keys($arMap)),true, array('UF_*' ));
    while ($arSection = $rsSections->GetNext())
    {
        $key = $arMap[$arSection['ID']];
        $arResult['SECTIONS'][$key]['PROPERTIES'] = $arSection;
    }
}
